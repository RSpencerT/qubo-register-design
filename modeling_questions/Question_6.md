...
For each layout `l`, first check whether it has at least `N` available sites. If not, skip it.
...
1. Sort atom pairs `(i,j)` by decreasing `|Q[i,j]|`.
2. Sort available site pairs `(s,t)` by decreasing `I[l,s,t]`.
...
* the selected layout `l`;
* the atom-to-site assignment `pi(i)`;
* the induced interaction matrix `Q_hat`;
* the objective value.

These values can initialize the binary variables `u[l]` and `a[l,i,s]` in the MIQP model before calling a solver such as Gurobi.
